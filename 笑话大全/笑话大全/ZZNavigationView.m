...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        leftBtn.frame = CGRectMake(8, self.frame.size.height/2-_leftImage.size.height/2, _leftImage.size.width/2, _leftImage.size.height/2);
		...
		_cleanBtn.frame = CGRectMake(self.frame.size.width-60, 5, 60, navigationBack.frame.size.height-10);
		[_cleanBtn setTitle:@"清理缓存" forState:UIControlStateNormal];
		_cleanBtn.titleLabel.font = [UIFont boldSystemFontOfSize:13];
		[navigationBack addSubview:_cleanBtn];
	}
	return self;
}
...
{
    leftBtn.frame = CGRectMake(8-10, navigationBack.frame.size.height/2-leftImage.size.height/2+2.5, leftImage.size.width-10, leftImage.size.height-5);
